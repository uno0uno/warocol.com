        ...
        <div class="border-b-2 border-border p-6">
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              ...
              <div>
                <h2 class="text-xl font-bold text-text-primary">Completar Precios de Cotización</h2>
                <p class="text-sm text-text-secondary">Agrega los precios confirmados por el proveedor</p>
              </div>
            </div>
            <button
              @click="closeModal"
        ...
        <form @submit.prevent="handleSubmit" class="p-6 space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              rows="3"
              class="input-base w-full px-4 py-2"
              placeholder="Notas sobre los precios o condiciones del proveedor..."
            ></textarea>
          </div>
          ...
          <div class="flex justify-end space-x-3 pt-4 border-t-2 border-border">
            ...
            >
              ...
              <span>{{ loading ? 'Completando...' : 'Completar Cotización' }}</span>
            </button>
          </div>
        </form>
      </div>
...
watch(() => props.isOpen, (newValue) => {
  if (newValue) {
    priceItems.value = props.items.map(item => ({
      id: item.id,
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await $fetch(`/api/suppliers/purchases/${props.purchaseId}/complete-quotation`, {
      method: 'POST',
      body: {
