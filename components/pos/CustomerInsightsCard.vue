...
<template>
  <div class="bg-surface rounded-2xl shadow-lg border border-border overflow-hidden">
    ...
    <div class="px-5 py-4 border-b border-border flex items-center gap-3">
      <div class="w-10 h-10 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-sm flex-shrink-0 select-none">
      ...
      <div class="min-w-0">
        ...
        <p class="text-xs text-text-tertiary leading-tight mt-0.5">{{ customerPhone }}</p>
      </div>
    </div>
      ...
      <div class="bg-surface-secondary rounded-xl p-4 flex flex-col gap-2">
        ...
        <div>
          <p class="text-sm font-bold text-text-primary leading-tight">
            {{ insights.last_order_date ? formatRelativeDate(insights.last_order_date) : '—' }}
          </p>
    ...
    <!-- Más pide -->
    <div v-if="insights.top_product_name" class="px-4 pb-4">
      <div class="bg-primary/5 border border-primary/15 rounded-xl p-4">
        ...
        <p v-if="insights.top_product_count" class="text-xs text-text-secondary mt-1">
