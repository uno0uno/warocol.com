    ...
    <div class="xl:col-span-2 space-y-6">
      ...
      <div class="bg-surface border-2 border-border rounded-xl p-6 md:p-8 shadow-sm">
        ...
        >
          ...
          <p class="font-semibold text-amber-900 mb-1">No elegible para vacaciones</p>
          <p class="text-sm text-amber-700">
            La compensación de vacaciones aplica únicamente a empleados con contrato laboral o jornaleros.
        ...
        <!-- Vacaciones form — only for employee and daily -->
        <form v-else-if="employee" @submit.prevent="handleSubmit" novalidate>
          <div class="space-y-6">
            <h3 class="text-lg font-semibold text-text-primary">Compensación de Vacaciones</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p v-if="paidAnios.includes(form.anio)" class="text-xs text-red-600 mt-1">
                  Este año ya tiene compensación de vacaciones registrada para este empleado.
                </p>
            ...
            <!-- Days worked + Vacaciones preview -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
              ...
              <div>
                ...
                <div class="px-4 py-2 bg-emerald-50 border border-emerald-200 rounded-lg">
                  <span class="text-xl font-bold text-emerald-700">{{ formatCurrency(vacacionesPreview) }}</span>
                  <p class="text-xs text-emerald-600 mt-0.5">
                    {{ form.gross_salary
                      ? `$${form.gross_salary.toLocaleString('es-CO')} × ${form.days_worked || 360} días ÷ 720`
            ...
            <!-- Payment method -->
            <div>
              <label class="block text-sm font-medium text-text-primary mb-3">Método de Pago *</label>
            ...
            <div>
              ...
              <input
                ...
                required
                class="input-base w-full px-4 py-2"
                aria-label="Fecha de pago de vacaciones"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Notas adicionales sobre esta compensación (opcional)"
            ...
            <div class="flex gap-3 pt-2">
              ...
              >
                ...
                <span>{{ isSubmitting ? 'Registrando...' : 'Registrar Vacaciones' }}</span>
              </button>
              <NuxtLink
                :to="`/equipo/salarios/${employeeId}`"
      ...
      <!-- History Section -->
      <div class="bg-surface border-2 border-border rounded-xl p-6 md:p-8 shadow-sm">
        <h3 class="text-lg font-semibold text-text-primary mb-4">Historial de Vacaciones</h3>
        ...
        <UiResponsiveDataView
          v-else
          :data="vacacionesHistory"
          :columns="historyColumns"
          item-key="id"
          empty-message="Sin compensaciones de vacaciones registradas"
          empty-sub-message="Los pagos de vacaciones aparecerán aquí"
        >
          <!-- Desktop cell slots -->
          <template #cell-anio="{ item }">
            <span class="font-medium text-text-primary">{{ item.anio }}</span>
          </template>
          <template #cell-vacaciones_amount="{ item }">
            <span class="font-semibold text-emerald-700">{{ formatCurrency(Number(item.vacaciones_amount)) }}</span>
          </template>
          ...
          <template #card="{ item }">
            <div class="bg-surface border border-border rounded-xl p-4 space-y-2">
              <div class="flex items-center justify-between">
                <span class="font-semibold text-text-primary">Vacaciones {{ item.anio }}</span>
                <span class="font-bold text-emerald-700 text-lg">{{ formatCurrency(Number(item.vacaciones_amount)) }}</span>
              </div>
      ...
      <div class="bg-surface border-2 border-border rounded-xl p-6 shadow-sm sticky top-6">
        ...
        <div class="bg-background rounded-lg p-4 border border-border space-y-3">
          ...
          <div class="pt-3 border-t border-border">
            <p class="text-sm text-text-secondary mb-1">Vacaciones a Compensar</p>
            <p class="text-2xl font-bold text-emerald-600">{{ formatCurrency(vacacionesPreview) }}</p>
            <p class="text-xs text-text-tertiary mt-0.5">Asiento: DR 2625 / CR Bancos</p>
          </div>
        </div>

        <!-- Info box -->
        <div class="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-3">
          <p class="text-xs text-blue-800 font-medium mb-1">Vacaciones (Ley colombiana)</p>
          <p class="text-xs text-blue-700">
            15 días hábiles remunerados por año trabajado.
            La compensación en dinero equivale a (salario × días trabajados) ÷ 720.
          </p>
        </div>
      </div>
    </div>
...
useHead({ title: 'Vacaciones - Equipo' })
...
const { data: employeeData } = useAsyncData(
  `employee-vacaciones-${employeeId}`,
...
// Fetch vacaciones payment history
const { data: historyData, pending: isLoadingHistory, refresh: refreshHistory } = useAsyncData(
  `vacaciones-history-${employeeId}`,
  () => $fetch(`/api/salaries/employees/${employeeId}/vacaciones`),
  {
    server: false,
    default: () => ({ data: [] }),
  }
...
const vacacionesHistory = computed(() => (historyData.value as any)?.data || [])

// Set of años already paid
const paidAnios = computed<number[]>(() =>
  vacacionesHistory.value.map((p: any) => Number(p.anio))
)

// Reactive vacaciones calculation: (gross_salary * days_worked) / 720
const vacacionesPreview = computed(() => {
...
const historyColumns = [
  ...
  { key: 'vacaciones_amount', label: 'Vacaciones pagadas', sortable: false },
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await $fetch(`/api/salaries/employees/${employeeId}/vacaciones`, {
      method: 'POST',
      body: payload,
    })
    ...
    if (status === 409) {
      errorMessage.value = `Las vacaciones del año ${form.anio} ya fueron registradas para este empleado.`
    } else {
      errorMessage.value = err?.data?.detail || 'Error al registrar las vacaciones'
    }
    console.error('Error recording vacaciones:', err)
  } finally {
