  ...
  <div class="page-layout">
    ...
    <template v-if="!cierreSuccess">
      ...
      <template v-if="currentStep === 1">
        ...
        <template v-else-if="xPreviewData">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
            ...
            <div class="bg-surface border-2 border-border rounded-lg">
              ...
              <div class="divide-y divide-border">
                ...
                <!-- Cortesias (#2670, #2683): tenant currency + i18n, no tocan caja. -->
                <div v-if="(xPreviewData.courtesyUnits ?? 0) > 0" class="flex justify-between px-4 py-2.5 text-sm"><span class="text-text-secondary">{{ t('finanzas.arqueo.courtesiesShort', { orders: xPreviewData.courtesyOrders ?? 0 }) }}</span><span class="font-medium">{{ t('finanzas.arqueo.courtesiesUnits', { units: xPreviewData.courtesyUnits }) }} · {{ (xPreviewData.courtesyCost ?? 0) > 0 ? formatCurrency(xPreviewData.courtesyCost) : formatCurrency(0) }}</span></div>
              </div>
            </div>
            <!-- Caja -->
            <div class="bg-surface border-2 border-border rounded-lg">
            ...
            <div v-if="(xPreviewData.breakdown ?? []).length > 0" class="sm:col-span-2 bg-surface border-2 border-border rounded-lg">
              ...
              <div class="overflow-x-auto">
                <div class="grid min-w-[680px] grid-cols-[1.35fr_.95fr_1fr_1fr_1fr] border-b border-data-table-border bg-data-table-header-bg text-xs font-semibold uppercase tracking-wide text-data-table-header-text">
                  ...
                  <div class="border-r border-dashed border-data-table-border/60 px-3 py-2">
                    <UiTableHeaderFilter
                      v-if="breakdownTipoHeaderOptions.length > 1"
                      v-model="breakdownTipoFilter"
                      :title="t('finanzas.common.type')"
                      filter-type="select"
                      :options="breakdownTipoHeaderOptions"
                      :all-label="t('finanzas.arqueo.allPaymentTypes')"
                    />
                    <span v-else>{{ t('finanzas.common.type') }}</span>
                  </div>
          ...
          <div class="flex gap-3">
            ...
            >
              {{ t('finanzas.arqueo.next') }} →
            </button>
          </div>
        </template>
        <div v-else-if="xPreviewError" class="text-sm text-text-secondary py-4">{{ t('finanzas.arqueo.loadPeriodSummaryError') }}</div>
      ...
      <div v-else-if="currentStep === 4" class="bg-surface border-2 border-border rounded-lg p-3 sm:p-4">
        <h3 class="text-sm font-semibold text-text-primary mb-3">{{ t('finanzas.arqueo.daySummary') }}</h3>
          ...
          <div v-if="nonCashMethods.length > 0" class="sm:col-span-2 bg-background rounded-lg border border-border overflow-hidden">
            ...
            <div class="overflow-x-auto">
              <div class="grid min-w-[760px] grid-cols-[1.2fr_.85fr_1fr_1fr_1fr] border-b border-data-table-border bg-data-table-header-bg text-xs font-semibold uppercase tracking-wide text-data-table-header-text">
                ...
                <span class="border-r border-dashed border-data-table-border/60 px-3 py-2 text-end">{{ t('finanzas.arqueo.reported') }}</span>
                <span class="px-3 py-2 text-end">{{ t('finanzas.common.difference') }}</span>
              </div>
              <div
                v-for="(method, index) in filteredNonCashMethods"
        ...
        <!-- Notas -->
        <div class="mb-3">
          <label class="text-xs font-medium text-text-secondary block mb-1">{{ t('finanzas.common.notes') }} <span class="font-normal">{{ t('finanzas.common.optional') }}</span></label>
          <textarea
            v-model="notes"
            :placeholder="t('finanzas.arqueo.notesPlaceholder')"
            rows="2"
      ...
      <div v-else-if="currentStep === 5" class="bg-surface border-2 border-border rounded-lg p-3 sm:p-4">
          ...
          <div v-if="nonCashMethods.length > 0" class="bg-background rounded-lg border border-border overflow-hidden lg:col-span-1">
            ...
            <div class="overflow-x-auto">
              <div class="grid min-w-[620px] grid-cols-[1.15fr_.8fr_1fr_1fr] border-b border-data-table-border bg-data-table-header-bg text-xs font-semibold uppercase tracking-wide text-data-table-header-text">
                ...
                <span class="border-r border-dashed border-data-table-border/60 px-3 py-2 text-end">{{ t('finanzas.arqueo.expected') }}</span>
                <span class="px-3 py-2 text-end">{{ t('finanzas.arqueo.reported') }}</span>
              </div>
              <div
                v-for="(method, index) in filteredNonCashMethods"
                :key="method.key"
        ...
        >
          ...
          <div>
            ...
            <p class="text-xs mt-0.5 transition-colors" :class="confirmArmed ? 'text-destructive/80' : 'text-state-warning-text/90'">
              {{ t('finanzas.arqueo.closeLockNotice') }}
            </p>
          </div>
        </div>
        ...
        <div class="flex gap-3">
          ...
          >
            ...
            <span v-if="isSubmitting">{{ t('finanzas.arqueo.closing') }}</span>
            <span v-else-if="confirmArmed">{{ t('finanzas.arqueo.confirmCashCount') }}</span>
            <span v-else>{{ t('finanzas.arqueo.closeDay') }}</span>
          </button>
        </div>
      </div>
    </template>
...
const breakdownTipoHeaderOptions = computed(() => {
