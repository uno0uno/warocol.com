...
<template>
  <div class="billing-page-root">
    ...
    <div v-else class="flex flex-col gap-3 md:gap-4">
      ...
      >
        <template #card="{ item }">
          <div v-if="item" class="bg-surface border border-border rounded-xl p-4 cursor-pointer hover:bg-surface-secondary transition-colors" @click="openEventDetail(item)">
            <div class="flex justify-between items-start mb-3">
              <p class="text-sm text-text-secondary">{{ formatDateTime(item.created_at) }}</p>
              <span :class="['text-xs font-medium px-2.5 py-1 rounded-full', eventStyle(item.event_type).badge]">
                {{ eventStyle(item.event_type).label }}
              </span>
            </div>
            <div class="flex justify-between items-center pt-2 border-t border-border">
              <span class="text-xs font-mono text-text-secondary truncate max-w-[160px]">{{ eventReference(item) }}</span>
              <span v-if="eventAmount(item)" class="text-base font-bold text-text-primary">{{ eventAmount(item) }}</span>
