  ...
  <div class="w-full">
    ...
    <div v-if="isSubmitting" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-8 flex flex-col items-center">
        ...
        <p class="mt-4 text-lg font-semibold text-text-primary">Actualizando combo...</p>
      </div>
    </div>
        ...
        <div class="p-4 sm:p-6">
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3">
              ...
              <div class="space-y-1">
                <p class="text-xs font-medium text-text-secondary uppercase tracking-wide">Editar Combo</p>
        ...
        <div v-else-if="currentStep === 3" key="step-3" class="bg-surface border border-border rounded-lg">
          ...
          <div class="border-b border-border p-4 sm:p-6 md:p-8">
            <div class="flex flex-col sm:flex-row justify-between items-start gap-4">
              <div>
                <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-text-primary mb-2">EDITAR COMBO</h1>
                <p class="text-xs sm:text-sm text-text-secondary">Resumen de los cambios</p>
              </div>
            </div>
          </div>
...
const comboId = route.params.id as string

const { data: comboData, pending: isLoadingCombo } = useAsyncData(
  `combo-${comboId}`,
  () => $fetch(`/api/menu/combos/${comboId}`),
  {
    server: false
  }
)
...
const isLoadingData = computed(() => {
  return isLoadingCombo.value || !categoriesData.value || !productsData.value
})
  ...
  if (data?.data) {
    ...
    form.value = {
      ...
      items: combo.items?.map((item: any) => ({
        ...
        individual_price: Number(item.individual_price || 0),
...
function updateItemPrices(index: number) {
  ...
  if (product) {
    ...
    // Set combo price to same as individual by default if not set
...
async function submitCombo() {
  ...
  try {
    ...
    await $fetch(`/api/menu/combos/${comboId}`, {
      method: 'PUT',
      body: form.value
    })
    ...
    alert(`Error al actualizar el combo: ${error.message || 'Por favor intenta de nuevo.'}`)
  } finally {
