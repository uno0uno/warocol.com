  ...
  <div>
    ...
    <div v-else class="flex flex-col gap-4">
      ...
      <div class="flex items-center justify-between gap-3">
        ...
        <button
          type="button"
          class="h-10 px-4 rounded-lg bg-primary text-sm font-semibold text-white hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 active:scale-[0.98] transition-all shadow-sm shadow-primary/30"
      ...
      <div v-else class="flex flex-col gap-2">
        <div
          ...
          :key="table.id"
          class="flex flex-col gap-2 rounded-xl border border-border bg-surface px-4 py-3"
        >
          <!-- Main row -->
          <div class="flex items-center gap-3">
            <!-- Info -->
            <div class="flex-1 min-w-0 flex items-center gap-3 flex-wrap">
              <span class="text-base font-bold text-text-primary">{{ table.name }}</span>
              <UiStatusBadge :variant="badgeVariant(table.status)" size="sm">
                {{ statusLabel(table.status) }}
              </UiStatusBadge>
              <span v-if="table.capacity" class="text-sm text-text-secondary">
          ...
          <div
            v-if="confirmingId === table.id"
            class="flex items-center gap-3 pt-1 pb-0.5 border-t border-border/60"
          >
            <p class="flex-1 text-sm text-text-secondary leading-snug">
              ¿Desactivar esta mesa? Desaparecerá del salón pero no se eliminarán sus datos.
            </p>
            <div class="flex items-center gap-2 flex-shrink-0">
              <button
                type="button"
                class="h-9 px-3 rounded-lg border border-border bg-surface text-xs font-medium text-text-secondary hover:bg-surface-secondary transition-colors focus:outline-none focus:ring-2 focus:ring-primary/20"
                @click="cancelConfirm"
              >
                Cancelar
              </button>
              <button
                type="button"
                :disabled="deletingId === table.id"
                class="h-9 px-3 rounded-lg bg-destructive text-xs font-semibold text-white hover:bg-destructive/90 transition-colors focus:outline-none focus:ring-2 focus:ring-destructive disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          <!-- 409 / error message -->
          <p v-if="deleteErrors[table.id]" class="text-sm text-destructive bg-destructive/10 rounded-lg px-3 py-2 flex items-start gap-2">
